...
int main()
{
  ...
  PID pid;
  // Parameters determined by trial and error, and some use of Twiddle
  // algorithm described in PID lectures.
  pid.Init(0.288778, 0.000904356, 0.00537731, false);

  double previous_time = clock() / CLOCKS_PER_SEC;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "")
      {
        ...
        if (event == "telemetry")
        {
          ...
          double steer_value = pid.CalculateSteering(dt);
